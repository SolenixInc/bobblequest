...
async function main() {
  ...
  const [authInfo, diff, commits, rawJson] = await Promise.all([
    ...
    run(
      `gh api graphql -F branch="${branch}" -F owner="${repoOwner}" -F repo="${repoName}" -f query='${gqlQuery}'`,
    ),
  ])

  if (!diff) {
    console.error(`No active PR found for branch: ${branch}`)
    process.exit(1)
  }
  ...
  const general = pr.comments.nodes
  if (general.length > 0) {
    console.log('\nGENERAL COMMENTS:')
    general.forEach((c) => {
  ...
  pr.reviews.nodes.forEach((review) => {
    if (review.body) {
      const icon = review.state === 'APPROVED' ? 'APPROVED' : 'REVIEW'
  ...
  const topLevelThreads = allInlineComments.filter((c) => !c.replyTo)
  ...
  topLevelThreads.forEach((c) => {
    ...
    const fileInfo = c.path
      ...
      : range
        ...
        : ''
    const minimized = c.isMinimized ? ` (Minimized: ${c.minimizedReason})` : ''
    console.log(`\nCOMMENT ${minimized}${c.author.login} | ${c.createdAt} ${fileInfo}\n${c.body}`)
    printThread(c.id)
  })
