...
# Configurable AI CLI for headless inference (default: claude)
...
# Use the repository's local .agents/tmp directory for ephemeral worktrees and logs
pr_dir="${base_dir}/.agents/tmp/async-reviews/pr-${pr_number}"
...
git branch -D "async-pr-review-${pr_number}" >> "${log_dir}/setup.log" 2>&1 || true
git worktree prune >> "${log_dir}/setup.log" 2>&1 || true

echo "Fetching PR #${pr_number}..." | tee -a "${log_dir}/setup.log"
if ! git fetch origin -f "pull/${pr_number}/head:async-pr-review-${pr_number}" >> "${log_dir}/setup.log" 2>&1; then
...
if [[ ! -d "${target_dir}" ]]; then
  ...
  if ! git worktree add "${target_dir}" "async-pr-review-${pr_number}" >> "${log_dir}/setup.log" 2>&1; then
    echo 1 > "${log_dir}/setup.exit"
...
# Resolve the AI CLI binary
AI_CLI_CMD="$(command -v "${AI_CLI}" || echo "${AI_CLI}")"

# Build CLI-specific flags for headless invocation
build_cli_args() {
  local prompt="${1}"
  # All supported CLIs use -p for headless prompt
  echo "-p"
  echo "${prompt}"
}
...
echo "  [3/5] Starting AI code review..."
rm -f "${log_dir}/review.exit"
{ "${AI_CLI_CMD}" -p "/review-frontend ${pr_number}" > "${log_dir}/review.md" 2>&1; echo $? > "${log_dir}/review.exit"; } &
  ...
  for i in "${!tasks[@]}"; do
    ...
    if [[ -f "${log_dir}/${t}.exit" ]]; then
      ...
      if [[ "${task_exit}" == "0" ]]; then
        ...
        echo "  FAIL ${t}: FAILED (exit code ${task_exit})"
      fi
      task_done[${t}]=1
    else
      echo "  WAIT ${t}: RUNNING"
...
if ! "${AI_CLI_CMD}" -p "Read the review at ${log_dir}/review.md, the automated test logs at ${log_dir}/npm-test.log, and the manual test execution logs at ${log_dir}/test-execution.log. Summarize the results, state whether the build and tests passed based on ${log_dir}/build-and-lint.exit and ${log_dir}/npm-test.exit, and give a final recommendation for PR ${pr_number}." > "${log_dir}/final-assessment.md" 2>&1; then
