...
log_dir="${base_dir}/.agents/tmp/async-reviews/pr-${pr_number}/logs"
...
for task_info in "${tasks[@]}"; do
  ...
  if [[ -f "${exit_file}" ]]; then
    ...
    if [[ "${exit_code}" == "0" ]]; then
      echo "OK ${task_name}: SUCCESS"
    else
      echo "FAIL ${task_name}: FAILED (exit code ${exit_code})"
      echo "   Last lines of ${file_path}:"
      tail -n 3 "${file_path}" | sed 's/^/      /' || true
    fi
  elif [[ -f "${file_path}" ]]; then
    echo "WAIT ${task_name}: RUNNING"
    all_done=false
  else
    echo "SKIP ${task_name}: NOT STARTED"
    all_done=false
  fi
done
