...
1.  **Fetch Nightly Results**: Use the `gh` CLI to inspect the latest run from
    the nightly eval workflow if applicable.
...
3.  **Read Logs**:
    ...
    - Enable verbose debugging via the project's debug log environment variable
      (e.g., `export DEBUG_LOG_FILE="debug.log"`).
...
3.  **Assertion Fidelity**:
    ...
    - **Primary Fix Trigger**: Adjust tool descriptions, system prompts, or
      **modules that contribute to the prompt template**.
