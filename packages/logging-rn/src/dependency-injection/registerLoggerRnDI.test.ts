/**
 * @fileoverview Tests for registerLoggerRnDI - wires Logger into the DI container.
 */
...
import { LOGGER_DEPENDENCY_KEY, registerLoggerRnDI } from './registerLoggerRnDI.ts'

describe('registerLoggerRnDI', () => {
  ...
  test('wraps in AnalyticsBridgedLogger when analytics provided', () => {
    ...
    registerLoggerRnDI(container, { analytics: mockTracker })
    const logger = container.resolve(LOGGER_DEPENDENCY_KEY) as Logger
  ...
  test('uses provided context', () => {
    ...
    registerLoggerRnDI(container, { context })
    const logger = container.resolve(LOGGER_DEPENDENCY_KEY) as Logger
